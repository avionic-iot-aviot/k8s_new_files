...
if [ "$newstatus" = "tuntap_up" ]; then
    ...
    route add -net $VAR_DOCKER_NETWORK.1 netmask 255.255.255.255 dev $tuntap_intf
    iptables -t nat -A POSTROUTING -o eth0 -s $VAR_DOCKER_NETWORK.1/32 -j MASQUERADE
elif [ "$newstatus" = "tuntap_down" ]; then
    /sbin/iptables -t nat -D POSTROUTING -o eth0 -s $VAR_DOCKER_NETWORK.1/32 -j MASQUERADE
fi
